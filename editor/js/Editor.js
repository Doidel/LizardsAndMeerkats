var Editor = function () {
	...
	this.sceneHelpers = new Physijs.Scene();
...
Editor.prototype = {
	...
	startPlay: function ( ) {
		...
		this.positionRotationSaves = { };
	
		this.scene.traverse( function ( child ) {
		
			if ( child._physijs ) {
				
				this.positionRotationSaves[ child.id ] = child.position.toArray().concat( child.rotation.toArray() );
				this.positionRotationSaves[ child.id ][ 6 ] = child.mass;
				if (child.material._physijs.massmodifier) child.mass *= child.material._physijs.massmodifier;
		...
		console.log(this.positionRotationSaves);
	...
	resetPlay: function ( ) {
		...
		this.scene.traverse( function ( child ) {
			...
			if ( child._physijs ) {
				...
				if ( this.positionRotationSaves ) {
					var posRot = this.positionRotationSaves[ child.id ];
					child.position = new THREE.Vector3( posRot[0], posRot[1], posRot[2] );
					child.rotation = new THREE.Euler( posRot[3], posRot[4], posRot[5] );
					child.mass = posRot[6];
				}
				child.__dirtyPosition = true;
				child.__dirtyRotation = true;
