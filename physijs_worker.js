...
var physijs_worker_functions = function(sceneOnMessage){
    ...
    //replace event.data with data.
    public_functions.onmessage = function( data ) {
        ...
        if ( data instanceof Float32Array ) {
            ...
            switch ( data[0] ) {
                case MESSAGE_TYPES.WORLDREPORT:
                    worldreport = new Float32Array( data );
                    break;

                case MESSAGE_TYPES.COLLISIONREPORT:
                    collisionreport = new Float32Array( data );
                    break;

                case MESSAGE_TYPES.VEHICLEREPORT:
                    vehiclereport = new Float32Array( data );
                    break;

                case MESSAGE_TYPES.CONSTRAINTREPORT:
                    constraintreport = new Float32Array( data );
                    break;
            }
        ...
        if ( data.cmd && public_functions[data.cmd] ) {
            //if ( data.params.id !== undefined && _objects[data.params.id] === undefined && data.cmd !== 'addObject' && data.cmd !== 'registerMaterial' ) return;
            public_functions[data.cmd]( data.params );
        }
